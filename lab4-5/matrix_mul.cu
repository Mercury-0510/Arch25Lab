...
__global__ void MatrixMulSharedMemKernel(float *A, float *B, float *C, int wA, int wB) {
  ...
  for (int a = aBegin, b = bBegin;
    ...
    // Load the elements of the sub-matrix of A into As
    int aIndex = a + wA * ty + tx;
    if (tx < wA && ty < wA) {
        As[ty][tx] = A[aIndex];
    } else {
        As[ty][tx] = 0.0f;
...
int main(int argc, char* argv[]) {
  ...
  float msecPerMatrixMulShared;
  cudaEventElapsedTime(&msecPerMatrixMulShared, start, stop);
  msecPerMatrixMulShared /= nIter;
  printf("Shared Memory Kernel Elapsed Time: %.3f ms\n", msecPerMatrixMulShared);

  // Test global memory kernel
  cudaEventRecord(start,0);
  for (int j = 0; j < nIter; j++) {
    MatrixMulKernel<<<grid, block>>>(d_M, d_N, d_P, m);
  }
  cudaEventRecord(stop, 0);
  cudaEventSynchronize(stop);
  float msecPerMatrixMulGlobal;
  cudaEventElapsedTime(&msecPerMatrixMulGlobal, start, stop);
  msecPerMatrixMulGlobal /= nIter;
  printf("Global Memory Kernel Elapsed Time: %.3f ms\n", msecPerMatrixMulGlobal);
  printf("Speedup with shared memory: %.2fx\n", msecPerMatrixMulGlobal/msecPerMatrixMulShared);
  ...
  cudaEventRecord(stop, 0);
  cudaEventSynchronize(stop);
  float msecPerMatrixMulCUBLAS;
  cudaEventElapsedTime(&msecPerMatrixMulCUBLAS, start, stop);
  msecPerMatrixMulCUBLAS /= nIter;
  printf("CUBLAS Elapsed Time: %.3f ms\n", msecPerMatrixMulCUBLAS);
  printf("Speedup with CUBLAS over shared memory: %.2fx\n", msecPerMatrixMulShared/msecPerMatrixMulCUBLAS);
  printf("Speedup with CUBLAS over global memory: %.2fx\n", msecPerMatrixMulGlobal/msecPerMatrixMulCUBLAS);
#endif

  // Compute and print the performance for shared memory kernel
  double flopsPerMatrixMul = 2.0 * (double)m * (double)n * (double)k;
  double gigaFlops = (flopsPerMatrixMul * 1.0e-9f) / (msecPerMatrixMulShared / 1000.0f);
  printf("Shared Memory Performance= %.2f GFlop/s, Time= %.3f msec, Size= %.0f Ops\n",
		  gigaFlops,
		  msecPerMatrixMulShared,
		  flopsPerMatrixMul);

  // Compute and print the performance for global memory kernel
  double gigaFlopsGlobal = (flopsPerMatrixMul * 1.0e-9f) / (msecPerMatrixMulGlobal / 1000.0f);
  printf("Global Memory Performance= %.2f GFlop/s, Time= %.3f msec, Size= %.0f Ops\n",
		  gigaFlopsGlobal,
		  msecPerMatrixMulGlobal,
		  flopsPerMatrixMul);

#ifdef USE_CUBLAS
  // Compute and print the performance for CUBLAS
  double gigaFlopsCUBLAS = (flopsPerMatrixMul * 1.0e-9f) / (msecPerMatrixMulCUBLAS / 1000.0f);
